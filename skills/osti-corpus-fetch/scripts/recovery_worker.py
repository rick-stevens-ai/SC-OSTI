#!/usr/bin/env python3
"""DOI-based PDF recovery worker (canonical multi-writer SQLite pattern).
...
Can run as:
  - One-shot:   `python recovery_worker.py --batch 200`
  - Daemon:     `python recovery_worker.py --daemon --interval 600 --batch 500`
...
CRITICAL: this script runs CONCURRENTLY with bulk_fetch_purl.py against the same DB.
Every write+commit MUST go through db_exec / db_commit (retry-with-backoff shim).
A bare cur.execute() under contention crashes with 'database is locked'.

Required schema additions (run schema_recovery.sql first):
  - recovery_queue (osti_id PK, reason, status, attempts, last_*, resolved_*)
  - recovery_log   (log_id PK, osti_id, run_id FK, ts, strategy, doi, source_url,
                    http_status, bytes, sha256, saved_path, error, duration_ms)
"""
...
# ── Multi-writer SQLite retry shim ──────────────────────────────────────────
# Use these for EVERY write+commit. A bare cur.execute() under concurrent-writer
# contention will crash with sqlite3.OperationalError: database is locked.
...
def process_one(conn, cur, run_id, osti_id, reason, doi, year, inter_strategy_delay):
    """Returns (final_status, last_strategy, last_err)."""
    if not doi:
...
def run_batch(conn, cur, run_id, batch_size, inter_paper_delay, inter_strategy_delay):
    ...
    for i, (osti_id, reason, doi, year) in enumerate(rows, 1):
        ...
        final_status, last_strategy, last_err = process_one(
            conn, cur, run_id, osti_id, reason, doi, year, inter_strategy_delay)
